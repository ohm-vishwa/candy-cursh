...
export const useGameLogic = (data: any[][], setData: (data: any) => any) => {
        ...
        if (targetRow >= 0 &&
            ...
            Animated.parallel([sourceTileAnimationX, sourceTileAnimationY, targetTileAnimationX, targetTileAnimationY]).start(async () => {
                ...
                if (matches?.length > 0) {
                    ...
                    // Resetting the animated values to zero so items don't 'jump'
                    ...
                    if (!hasMoves) {
                        // Board is stuck - trigger a shuffle
                        let d = await handleShuffleAndClear(newGrid);
                        newGrid = d.grid;
                        totalClearedCandies += d.clearedMatching;

                        // Keep shuffling if the shuffle itself didn't create a move
                        while (!(await hasPossibleMoves(newGrid))) {
                            let p = await handleShuffleAndClear(newGrid);
                            newGrid = p.grid;
    ...
    const handleGesture = async (event: any,
        ...
        if (state === State.END) {
            ...
            const asbX = Math.abs(translationX);
